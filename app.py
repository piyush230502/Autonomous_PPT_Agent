...
try:
    ...
    st.warning(f"Could not load .env file: {e}. Please ensure API keys are set in Streamlit secrets or entered manually.")
...
def get_api_key(service_name, key_name):
    ...
    if key:
        return key
    # If still not found, prompt user (less secure, use for local testing only if needed)
    # st.warning(f"{service_name} API Key not found in secrets or environment variables. Please enter it below.")
    # key = st.text_input(f"Enter your {service_name} API Key:", type="password", key=f"api_key_{key_name.lower()}")
    # return key
...
def get_llm(provider, api_key):
    ...
    try:
        if provider == "Groq":
            ...
            return ChatGoogleGenerativeAI(model="gemini-1.5-flash", temperature=0.7) # Or gemini-pro
...
def setup_and_run_crew(topic, llm, serper_api_key):
    ...
    try:
        ...
        st.error(f"Failed to initialize SerperDevTool: {e}. Check your API key.")
        return None
...
st.markdown("Enter a topic, choose an LLM, provide API keys, and get a PowerPoint presentation!")
...
with st.sidebar:
    ...
    llm_provider = st.selectbox(
        ...
        help="Select the Language Model provider. Ensure you have the corresponding API key."
    )

    # API Key Inputs (Conditional)
    api_key = None
    if llm_provider == "Groq":
        api_key = st.text_input("Enter Groq API Key", type="password", key="groq_api_key", help="Get from console.groq.com")
        if not api_key: api_key = get_api_key("Groq", "GROQ_API_KEY")
    elif llm_provider == "Google Gemini":
        api_key = st.text_input("Enter Google API Key", type="password", key="google_api_key", help="Get from Google AI Studio")
        if not api_key: api_key = get_api_key("Google Gemini", "GOOGLE_API_KEY")
    elif llm_provider == "OpenAI":
        api_key = st.text_input("Enter OpenAI API Key", type="password", key="openai_api_key", help="Get from platform.openai.com")
        if not api_key: api_key = get_api_key("OpenAI", "OPENAI_API_KEY")

    # Serper API Key Input
    st.markdown("---")
    st.markdown("**Web Search Configuration**")
    serper_api_key = st.text_input("Enter Serper API Key", type="password", key="serper_api_key", help="Needed for web research. Get from serper.dev (free tier available)")
    if not serper_api_key:
        serper_api_key = get_api_key("Serper", "SERPER_API_KEY")

    st.markdown("---")
    st.info("API Keys can be stored in `.streamlit/secrets.toml` for deployment or a `.env` file for local use.")
...
if st.button("✨ Generate Presentation"):
    if not topic:
        ...
        st.warning("Please select an LLM provider in the sidebar.")
    elif not api_key:
        st.warning(f"Please enter the API key for {llm_provider} in the sidebar.")
    elif not serper_api_key:
        st.warning("Please enter the Serper API key in the sidebar for web research.")
    else:
        # Initialize LLM
        ...
        if llm:
            with st.spinner(f"Generating presentation on '{topic}' using {llm_provider}... This may take a few minutes."):
                ...
                if crew_result:
                    ...
                    st.text_area("Content Generated", crew_result if isinstance(crew_result, str) else str(crew_result), height=200)

                        # Try parsing based on type
                    if isinstance(crew_result, str):
                        parsed_slides = parse_crew_output(crew_result)
                    elif isinstance(crew_result, dict) and 'output' in crew_result:
                        parsed_slides = parse_crew_output(crew_result['output'])
                    else:
                        st.warning("CrewAI result format is unexpected. Attempting fallback parse.")
                        parsed_slides = parse_crew_output(str(crew_result))
                    ...
                    st.error("Failed to get results from CrewAI.")
